...
function validateForm(form, action) {
  ...
  form.querySelectorAll('input[required], select[required]').forEach(input => {
    if (!input.value.trim()) {
      ...
      input.setAttribute('title', `${input.previousElementSibling.textContent} is required.`); // Add hover message
      ...
      input.removeAttribute('title'); // Remove hover message if valid
    }
  });
  ...
  if (action === 'signup' || action === 'resetPassword') {
    ...
    if (password && !/^(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d!@#$%^&*()_+={}["]:;'<>.,?/\\-]{8,}$/.test(password.value.trim())) {
      ...
      password.setAttribute('title', "Password must be at least 8 characters long and contain both letters and numbers.");
  ...
  if (action === 'signup' && isValid) {
    ...
    if (phone && !/^\+?[0-9]{10,15}$/.test(phone.value.trim())) {
      ...
      phone.setAttribute('title', "Please enter a valid phone number.");
  ...
  if (action === 'login' && isValid) {
    ...
    if (!username || !username.value.trim()) {
      ...
      username.setAttribute('title', "Username is required.");
    ...
    if (!loginPassword || !loginPassword.value.trim()) {
      ...
      loginPassword.setAttribute('title', "Password is required.");
      showMessage("Password is required.", false);
      isValid = false;
    } else if (loginPassword) {
      loginPassword.style.borderColor = '';
      loginPassword.removeAttribute('title');
    }
  }
