...
const scriptURL = 'https://script.google.com/macros/s/AKfycbxqsRMcDYzZ3EHuQTyIiaYyCVml8HvNBJB2k_Ligd33BaiGG1PQpOqwNMX80hI74mlv/exec';
...
async function displayDashboard(username, password) {
  // Load data and toggle dashboard visibility
  await loadDashboardData(username, password);
  toggleDashboard();
 const buttons = [
...
async function loadDashboardData(username, password) {
  try {
    console.log(`Fetching dashboard data for username: ${username}`);

    const url = `${scriptURL}?action=login&username=${encodeURIComponent(username)}&loginPassword=${encodeURIComponent(password)}`;
    ...
    if (data.result === "success") {
      ...
      document.getElementById("user-email").textContent = dashboardData.email || "No Email Found";
      document.getElementById("user-username").textContent = dashboardData.username || "No Username";
      document.getElementById("school-name").textContent = dashboardData.schoolname || "No School Name";
      document.getElementById("school-index").textContent = dashboardData.schoolindex || "No School Index";

      const messageList = document.getElementById("message-list");
      messageList.innerHTML = `<li>${dashboardData.usermessage || "No Messages"}</li>`;
      document.getElementById("hover-icon2").textContent = dashboardData.usermessage ? 1 : 0;

      const notificationList = document.getElementById("notification-list");
      notificationList.innerHTML = `<li>${dashboardData.notification || "No Notifications"}</li>`;
      document.getElementById("hover-icon1").textContent = dashboardData.notification ? 1 : 0;

      showMessage("Dashboard loaded successfully", true);
    } else {
      console.error("Failed to retrieve dashboard data:", data.message);
      showMessage(data.message || "Failed to retrieve dashboard data", false);
    }
  } catch (error) {
    console.error("Error loading dashboard data:", error);
    showMessage("Error loading dashboard data. Please try again.", false);
  }
}
