...
function submitForm(action) {
  ...
  if (action === 'login') {
    ...
    // Show success message and toggle to login form
    showMessageWithOverlay(
      `${action.charAt(0).toUpperCase() + action.slice(1)} successful! Please log in.`, 
      true
    );
    toggleForms('lg-form');
  }
...
function showMessageWithOverlay(message, success = true, onComplete = null) {
 ...
 if (success) {
  ...
  overlay.className = "overlay-form-message"; // Assign CSS class to the overlay
