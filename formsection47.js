...
function validateForm(form, action) {
  ...
  // Helper function to create a text-form nmg-alert
  function setFormAlert(input, message) {
    let alert = input.nextElementSibling;
    if (!alert || !alert.classList.contains('nmg-alert')) {
      alert = document.createElement('span');
      alert.className = 'nmg-alert';
  ...
  // Helper function to remove text-form nmg-alert
  ...
  form.querySelectorAll('input[required], select[required]').forEach(input => {
    if (!input.value.trim()) {
      ...
      setFormAlert(input, `${input.previousElementSibling.textContent} is required.`); // Add alert message
      ...
      removeFormAlert(input); // Remove alert if valid
    }
  });
  ...
  if (action === 'signup' || action === 'resetPassword') {
    ...
    if (password && !/^(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d!@#$%^&*()_+={}["]:;'<>.,?/\\-]{8,}$/.test(password.value.trim())) {
      ...
      setFormAlert(password, "Password must be at least 8 characters long and contain both letters and numbers.");
      showMessage("Password must be at least 8 characters long and contain both letters and numbers.", false);
      isValid = false;
    } else if (password) {
      password.style.borderColor = '';
      removeFormAlert(password);
    }

    // Password match validation
    if (password && confirmPassword && password.value.trim() !== confirmPassword.value.trim()) {
      confirmPassword.style.borderColor = 'red';
      setFormAlert(confirmPassword, "Passwords do not match.");
      showMessage("Passwords do not match.", false);
      isValid = false;
    } else if (confirmPassword) {
      confirmPassword.style.borderColor = '';
      removeFormAlert(confirmPassword);
    }
  }
  ...
  if ((action === 'signup' || action === 'resetPassword') && isValid) {
    ...
    if (email && !/^\S+@\S+\.\S+$/.test(email.value.trim())) {
      ...
      setFormAlert(email, "Please enter a valid email address.");
      showMessage("Please enter a valid email address.", false);
      isValid = false;
    } else if (email) {
      email.style.borderColor = '';
      removeFormAlert(email);
    }
  }
  ...
  if (action === 'signup' && isValid) {
    ...
    if (phone && !/^\+?[0-9]{10,15}$/.test(phone.value.trim())) {
      ...
      setFormAlert(phone, "Please enter a valid phone number.");
      showMessage("Please enter a valid phone number.", false);
      isValid = false;
    } else if (phone) {
      phone.style.borderColor = '';
      removeFormAlert(phone);
    }
  }
