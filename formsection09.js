...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Initially, show the login form by default (since the page loads)
  ...
  if (email && token) {
    ...
    const resetForm = document.getElementById('new-password-form'); // Ensure this matches your form's ID
    if (resetForm) {
      resetForm.querySelector('input[name="email"]').value = email;
      resetForm.querySelector('input[name="token"]').value = token;
    }

    // Hide the homepage container (nmgContainerDiv)
