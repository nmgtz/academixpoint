document.addEventListener("DOMContentLoaded", () => {
  ...
  const toggleVisibility = () => {
    ...
    if (whatsapp && facebook && share && sms && globe && close) {
      ...
      globe.style.display = isGlbeVisible ? "none" : "block";
  ...
  // Initially, show the login form by default (in case no reset is detected)
  toggleForms('lg-form');

  if (email && token) {
    // If email and token are present in the URL, show the reset password form
    toggleForms('newSet-form'); // Show the reset password form (newSet-form)

    // Pre-fill email and token in the reset password form
    const resetForm = document.getElementById('newSet-form');
    if (resetForm) {
      resetForm.querySelector('input[name="email"]').value = email;
      resetForm.querySelector('input[name="token"]').value = token;
    }

    // Hide the homepage container (nmgContainerDiv) when password reset is needed
