...
accountButton.addEventListener("click", showPopup);
  ...
  if (loginButton && registerButton) {
    loginButton.addEventListener("click", (e) => {
      ...
      history.pushState(null,"",`user-logIn?action=logToAccount`);
      toggleForms("lg-form");
    });
    ...
    registerButton.addEventListener("click", (e) => {
      ...
      document.querySelector(".footer-bottom").style.display = "none";
      document.querySelector(".login-form").style.display = "block";
      history.pushState(null,"",`user-registration?action=creatingAccount`);
      toggleForms("sp-form");
    });
  } else {
...
function toggleForms(formIdToShow) {
  ...
  if (selectedForm.id === "lg-form") {
      history.pushState(null, "", `/user-logIn?action=logToAccount`);

    } else if (selectedForm.id === "sp-form") {
      history.pushState(null, "", `/user-registration?action=createAccount`);

    } else if (selectedForm.id === "set-form") {
      history.pushState(null, "", `/user-reset?action=resetingPassword`);

    } else if (selectedForm.id === "newSet-form") {
      history.pushState(null, "", `/user-newPassword?action=createNewPassword`);

    } else {
      // If no specific form is selected, ensure we show /home only if not already there
      if (window.location.pathname !== "/" && window.location.pathname !== "/home") {
        history.replaceState(null, "", "/");  // Displays `/home` when navigated explicitly to home
      }
    }
  }
}
...
function updateDashboardUI(data) {
  ...
  history.pushState(null,"",`user-loggedIn?ID=${data.schoolindex}`);
...
document.getElementById("assign-teachers").onclick = function() {
 ...
 if (userEmail) userEmail.textContent = data.email ? `Email Address: ${data.email}` : "Email not available";
  if (userUsername) userUsername.textContent = data.username ? `Username: ${data.username}` : "Username not available";
  if (schoolName) schoolName.textContent = data.schoolname ? `School Name: ${data.schoolname}` : "School name not available";
  if (schoolIndex) schoolIndex.textContent = data.schoolindex ? `School Index No: ${data.schoolindex}` : "School index not available";
