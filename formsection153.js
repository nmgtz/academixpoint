...
async function loadDashboardData(username, password) {
  try {
    ...
    const response = await fetch(`${scriptURL}?action=login&username=${username}&loginPassword=${password}`);
    ...
    if (data.result === "success") {
      ...
      messageList.innerHTML = `<li>${dashboardData.usermessage}</li>`;
      document.getElementById("hover-icon2").textContent = 1;

      const notificationList = document.getElementById("notification-list");
      notificationList.innerHTML = `<li>${dashboardData.notification}</li>`;
      document.getElementById("hover-icon1").textContent = 1;
    } else {
      console.error("Failed to retrieve dashboard data:", data.message);
      showMessage("Failed to retrieve dashboard data", false);
    }
  } catch (error) {
    console.error("Error loading dashboard data:", error);
    showMessage("Error loading dashboard data", false);
  }
}
