...
function toggleForms(formIdToShow, isLoggedin) {
  // Get the dashboard element
  const setDashboard = document.getElementById('set-dashboard');
  ...
  if (isLoggedin) {
    ...
    // Show the dashboard if transitioning to it
    if (formIdToShow === 'set-dashboard' && setDashboard) {
      setDashboard.style.display = 'block';
      return; // Exit function
    }

    // Handle transitioning from the dashboard to another form
    if (setDashboard && setDashboard.style.display === 'block') {
      setDashboard.style.display = 'none';
    }
  } else {
    // If not logged in, ensure the dashboard is hidden
    if (setDashboard) {
      setDashboard.style.display = 'none';
    }
  }
...
function submitForm(action) {
  ...
  if (action === 'login') {
   const isLoggedin = true; // Update login state
    ...
    setTimeout(() => {
     toggleForms('set-dashboard', isLoggedin);
      document.getElementById("lg-form").style.display = "none"; // Hide login form
      document.getElementById("set-dashboard").style.display = "block"; // Show dashboard
    }, 10000); // 10 seconds
...
function displayDashboard() {
  ...
  const nmgContainerDiv = document.getElementById('nmgContainerDiv');
  const loginFormDiv = document.querySelector(".login-form");

  // Hide the nmgContainerDiv and login form
  if (nmgContainerDiv) nmgContainerDiv.style.display = "none";
