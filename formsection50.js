...
function displayDashboard(dashboardData) {
  ...
  notificationList.innerHTML = dashboardData.notifications
    .map((notification) => `<li>${notification}</li>`)
    .join('');

  // Update notification count
  document.getElementById('hover-icon1').textContent = dashboardData.notifications.length;

  // Populate messages
  const messageList = document.getElementById('message-list');
  messageList.innerHTML = dashboardData.messages
    .map((message) => `<li>${message}</li>`)
    .join('');

  // Update message count
  document.getElementById('hover-icon2').textContent = dashboardData.messages.length;

  // Populate user dropdown
  document.getElementById('user-email').textContent = `Email: ${dashboardData.email}`;
  document.getElementById('user-username').textContent = `Username: ${dashboardData.username}`;
  document.getElementById('school-name').textContent = `School: ${dashboardData.schoolname}`;
  document.getElementById('school-index').textContent = `Index: ${dashboardData.schoolindex}`;
}
