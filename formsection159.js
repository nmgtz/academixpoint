...
async function loadDashboardData(username, password) {
  try {
    // Ensure that 'action' is passed correctly
    const action = 'login'; // Assuming 'login' is the action you want to perform
    ...
    if (data.result === "success") {
      ...
      document.getElementById('user-email').textContent = data.email || "Email not available";
      document.getElementById('user-username').textContent = data.username || "Username not available";
      document.getElementById('school-name').textContent = data.schoolname || "School name not available";
      document.getElementById('school-index').textContent = data.schoolindex || "School index not available";

      // Update messages and notifications...
      ...
      document.getElementById('hover-icon2').textContent = messages.length;
      document.getElementById('hover-icon1').textContent = notifications.length;
    } else {
