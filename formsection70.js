...
populateDropdown(dropdownId, items);
logout();
}
 ...
 // Function to toggle dropdown visibility
    ...
    // Function to update notification/message counters
    function updateCounter(elementId, count) {
      const counter = document.getElementById(elementId);
      counter.textContent = count > 99 ? "99+" : count;
    }

    // Function to populate dropdown lists dynamically
    function populateDropdown(dropdownId, items) {
      const list = document.getElementById(dropdownId);
      list.innerHTML = ''; // Clear existing items
      items.forEach((item) => {
        const li = document.createElement('li');
        li.textContent = item; // Customize item content
        list.appendChild(li);
      });
    }
