...
function toggleForms(formIdToShow) {
  ...
  const setDashboard = document.getElementById('set-dashboard');  // Make sure we target 'set-dashboard'

  // If transitioning to the dashboard
  if (formIdToShow === 'set-dashboard') {
    // Hide all forms
    ...
    // Show the dashboard
    if (setDashboard) {
      setDashboard.style.display = 'block';  // Show the dashboard container
    }
    return; // Exit function to prevent further toggling
  }

  // If transitioning from the dashboard to another form
  if (dashboard && dashboard.style.display === 'block') {
    setDashboard.style.display = 'none';  // Hide the dashboard container
  }
