...
const scriptURL = 'https://script.google.com/macros/s/AKfycbyNO5G6gWPsAusy3kky9BwHkiIINNYAyloqiO1am7uuIeuOXCn30UC2qFxrzKITOJqo/exec';
...
const schoolIndexes = document.querySelectorAll('.school-index');
...
userEmails.forEach(userEmail => {
  if (userEmail) {
    userEmail.textContent = data.email ? `Email Address: ${data.email}` : "Email not available";
  }
});

// Loop through each element for 'user-username' class
userUsernames.forEach(userUsername => {
  if (userUsername) {
    userUsername.textContent = data.username ? `${data.username}` : "Username not available";
  }
});

// Loop through each element for 'school-name' class
schoolNames.forEach(schoolName => {
  if (schoolName) {
    schoolName.textContent = data.schoolname ? `${data.schoolname}` : "School name not available";
  }
});

// Loop through each element for 'school-index' class
schoolIndexes.forEach(schoolIndex => {
  if (schoolIndex) {
    schoolIndex.textContent = data.schoolindex ? `${data.schoolindex}` : "School index not available";
  }
});
