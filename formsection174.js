  ...
  if (action === 'login') {
    ...
    setTimeout(() => {
     // Example Initialization Call
      initializeDashboard(username, password);
...
// Global Variables
let email, notification, phone, schoolindex, schoolname, usermessage, username;

// Function to load and process dashboard data
async function loadDashboardData(username, password) {
  try {
    ...
    schoolindex = dashboardData.schoolindex;
    schoolname = dashboardData.schoolname;
    usermessage = dashboardData.usermessage;
    ...
    if (dashboardData.result === "success") {
      ...
      console.error("Error:", dashboardData.message);
      showMessage(dashboardData.message || "Unknown error", false);
    }
  } catch (error) {
...
function updateDashboardUI(data) {
  ...
  if (notificationList) {
    const notifications = Array.isArray(data.notification) ? data.notification : [data.notification];
    notificationList.innerHTML = ""; // Clear existing notifications
    notifications.forEach(note => {
      const li = document.createElement('li');
      li.textContent = note || "No notifications available";
  ...
  if (messageList) {
    const messages = Array.isArray(data.usermessage) ? data.usermessage : [data.usermessage];
    messageList.innerHTML = ""; // Clear existing messages
    messages.forEach(msg => {
      const li = document.createElement('li');
      li.textContent = msg || "No messages available";
...
function showMessage(message, success = true) {
  const messageBox = document.getElementById('message-box'); // Example container for messages
  if (messageBox) {
    messageBox.textContent = message;
...
// Utility function to check if data is ready
function isDataReady() {
  return email !== undefined && phone !== undefined && schoolname !== undefined;
}

// Example Usage
async function initializeDashboard(username, password) {
  ...
  if (isDataReady()) {
    ...
    console.log("Dashboard data is not ready yet!");
  }
}
