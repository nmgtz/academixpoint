...
function displayDashboard() {
 ...
 const buttons = [
  ...
  const clickedInside = buttons.some(({ buttonId, targetClass }) => {
    ...
    // Ensure both button and target exist before calling `contains`
    return (button && button.contains(e.target)) || (target && target.contains(e.target));
  });
...
async function loadDashboardData(username, password) {
  try {
    ...
    // Display a loading state in the dashboard
    const dashboard = document.getElementById('dashboard');
    if (dashboard) {
      dashboard.innerHTML = '<p>Loading your dashboard...</p>'; // Temporary loading message
    }
...
function updateDashboardUI(data) {
  ...
  if (messageCount) messageCount.textContent = data.usermessage?.length || 0;
  if (notificationCount) notificationCount.textContent = data.notification?.length || 0;

  // Remove the temporary loading state
  const dashboard = document.getElementById('dashboard');
  if (dashboard) {
    dashboard.innerHTML = ""; // Clear the loading message
  }
}
